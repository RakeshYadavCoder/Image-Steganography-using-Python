...
`""""""`'''
    ...
    def main(self,root):
        ...
        f = Frame(root)
        f.configure(bg='#ff5500')

        title = Label(f,text='Image Steganography', bg='#ff5500')
        title.config(font=('courier',33))
        title.grid(pady=10)

        b_encode = Button(f,text="Encode",bg="#0099ff",command= lambda :self.frame1_encode(f), padx=14)
        b_encode.config(font=('courier',14))
        b_decode = Button(f, text="Decode",bg='#00cba9',padx=14,command=lambda :self.frame1_decode(f))
        b_decode.config(font=('courier',14))
        b_decode.grid(pady = 12)

        ascii_art = Label(f,text=self.art, bg='#ff5500')
        # ascii_art.config(font=('MingLiU-ExtB',50))
        ascii_art.config(font=('courier',60))

        ascii_art2 = Label(f,text=self.art2, bg='#ff5500')
        # ascii_art.config(font=('MingLiU-ExtB',50))
    ...
    def frame1_decode(self,f):
        ...
        d_f2.configure(bg='#ffd700')
        label_art = Label(d_f2, text='٩(^‿^)۶',bg='#ffd700')
        label_art.config(font=('courier',90))
        label_art.grid(row =1,pady=50)
        l1 = Label(d_f2, text='Select Image with Hidden text:',bg='#ffd700')
        l1.config(font=('courier',18))
        l1.grid()
        bws_button = Button(d_f2, text='Select', command=lambda :self.frame2_decode(d_f2),bg='Green')
        bws_button.config(font=('courier',18))
        bws_button.grid()
        back_button = Button(d_f2, text='Cancel', command=lambda : Stegno.home(self,d_f2),bg='Red')
    ...
    def frame2_decode(self,d_f2):
        ...
        if not myfile:
            ...
            l4= Label(d_f3,text='Selected Image :',bg="#e7008a")
            ...
            l2 = Label(d_f3, text='Hidden data is :',bg="#e7008a")
            ...
            text_area.grid()
            back_button = Button(d_f3, text='Cancel', command= lambda :self.page3(d_f3),bg="Red")
            back_button.config(font=('courier',11))
            back_button.grid(pady=15)
            back_button.grid()
            show_info = Button(d_f3,text='More Info',command=self.info,bg="#0099ff")
    ...
    def frame1_encode(self,f):
        ...
        f2 = Frame(root)
        root.configure(bg='#ffd700')
        f2.configure(bg='#ffd700')
        label_art = Label(f2, text='\'\(°Ω°)/\'',bg='#ffd700')
        label_art.config(font=('courier',70))
        label_art.grid(row =1,pady=50)
        l1= Label(f2,text='Select the Image in which \nyou want to hide text :', bg='#ffd700')
        l1.config(font=('courier',18))
        l1.grid()


        bws_button = Button(f2,text='Select',bg='Green',command=lambda : self.frame2_encode(f2))
        bws_button.config(font=('courier',18))
        bws_button.grid()
        back_button = Button(f2, text='Cancel',bg='Red', command=lambda : Stegno.home(self,f2))
        back_button.config(font=('courier',18))
        back_button.grid(pady=15)
        back_button.grid()
        f2.grid()
    ...
    def frame2_encode(self,f2):
        ...
        if not myfile:
            ...
            myimage = myimg.resize((300,200))
            img = ImageTk.PhotoImage(myimage)
            l3= Label(ep,text='Selected Image',bg="#e7008a")
            ...
            panel.grid()
            l2 = Label(ep, text='Enter the message',bg="#e7008a")
            l2.config(font=('courier',18))
            l2.grid(pady=15)
            text_area = Text(ep, width=50, height=10)
            text_area.grid()
            encode_button = Button(ep, text='Cancel',bg="Red", command=lambda : Stegno.home(self,ep))
            encode_button.config(font=('courier',11))
            data = text_area.get("1.0", "end-1c")
            back_button = Button(ep, text='Encode', bg="Green",command=lambda : [self.enc_fun(text_area,myimg),Stegno.home(self,ep)])
            back_button.config(font=('courier',11))
            back_button.grid(pady=15)
            encode_button.grid()
            ep.grid(row=1)
